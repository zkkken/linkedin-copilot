 ...
 *
 * Display character counts and LinkedIn limits
 */
...
export const CharacterCount: React.FC<CharacterCountProps> = ({ content, sectionType }) => {
  ...
  if (!limit) {
    // No limit configured, just show the character count
    return (
    ...
    <div className="mt-1 space-y-1">
      ...
      {status === 'error' && (
        <p className="text-xs text-red-600 flex items-center mt-1">
          ...
          Over the LinkedIn limit — trim the content.
        </p>
      )}
      {status === 'warning' && (
        <p className="text-xs text-amber-600 flex items-center mt-1">
          ...
          Getting close to the limit — consider tightening the copy.
        </p>
      )}
    </div>
  );
